<template>
  <div @mouseover="floatVisible=true" @mouseout="floatVisible=false" class="msg-top-content">
      ...
      <div class="msg-content">
        ...
        >
          <template v-for="(item, index) of msgList">
            <el-tooltip
              v-if="item.NOTIFICATIONS.length>19"
              :key="index"
              class="item"
              effect="dark"
            ...
            <template v-else>
              <div :title="item.NOTIFICATIONS" @click="previewMsg(item)" class="infinite-list-item" :key="index">
                <i :class="item.NOTICE_TYPE==2?'el-icon-s-comment':'el-icon-finished'"></i>
                {{item.NOTIFICATIONS}}
              </div>
    ...
    previewMsg(item) {
      if (item.NOTICE_TYPE == 2) {
        ...
        if (item.NOTICE_STATUS != 2) {
          //筛选出已读消息
          this.axios
            .post(this.serverIP + "SwcSysNotice/getSwcSysNoticeReadStatus", {
